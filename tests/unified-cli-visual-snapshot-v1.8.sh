#!/usr/bin/env bash
# NanoBK Proxy Suite — v1.8.6 CLI Visual Snapshot Test
#
...
echo ""
echo "=== Test Suite: v1.8.6 CLI Visual Snapshot ==="
...
output=$(source_ui_and_run "NANOBK_PLAIN=1" "
  ui_banner 'v1.8.6' 'Full Recommended — VPS + Cloudflare + Bot + Web Panel'
")

assert_contains "$output" "NanoBK Proxy Suite" "Banner: product name"
assert_contains "$output" "v1.8.6" "Banner: version"
