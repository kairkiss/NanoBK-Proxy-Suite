...
if run_with_timeout 15 bash "$NANOBK" beginner gate dns >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate cert >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate token >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate dns --json >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate cert --json >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate token --json >/dev/null 2>&1; then
  ok "A7: nanobk beginner gate token --json exits 0"
else
  fail "A7: nanobk beginner gate token exits 0"
fi
...
DNS_TEXT=$(run_with_timeout 15 bash "$NANOBK" beginner gate dns 2>&1)
CERT_TEXT=$(run_with_timeout 15 bash "$NANOBK" beginner gate cert 2>&1)
TOKEN_TEXT=$(run_with_timeout 15 bash "$NANOBK" beginner gate token 2>&1)
...
DNS_JSON=$(run_with_timeout 15 bash "$NANOBK" beginner gate dns --json 2>&1)
CERT_JSON=$(run_with_timeout 15 bash "$NANOBK" beginner gate cert --json 2>&1)
TOKEN_JSON=$(run_with_timeout 15 bash "$NANOBK" beginner gate token --json 2>&1)
...
if run_with_timeout 15 bash "$NANOBK" beginner gate dns --apply >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate dns --yes >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate dns --confirm "test" >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate cert --issue >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate cert --yes >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate token --rotate >/dev/null 2>&1; then
...
if run_with_timeout 15 bash "$NANOBK" beginner gate token --yes >/dev/null 2>&1; then
...
# ── Section G: Bounded smoke checks for prior versions ─────────────────────

echo ""
echo "=== Section G: Prior version smoke checks ==="

# G50. v2.4.4 test file exists and basic command exits 0
V244_TEST="$REPO_DIR/tests/v2.4.4-ip-friendly-ux.sh"
if [[ -f "$V244_TEST" ]]; then
  ok "G50: v2.4.4 test file exists"
else
  fail "G50: v2.4.4 test file missing"
fi

# G51. v2.4.3 test file exists
V243_TEST="$REPO_DIR/tests/v2.4.3-subdomain-conflict-ux.sh"
if [[ -f "$V243_TEST" ]]; then
  ok "G51: v2.4.3 test file exists"
else
  fail "G51: v2.4.3 test file missing"
fi

# G52. v2.4.2 test file exists
V242_TEST="$REPO_DIR/tests/v2.4.2-beginner-flow-renderer.sh"
if [[ -f "$V242_TEST" ]]; then
  ok "G52: v2.4.2 test file exists"
else
  fail "G52: v2.4.2 test file missing"
fi

# G53. v2.4.0 test file exists
V240_TEST="$REPO_DIR/tests/v2.4.0-beginner-production-setup-scope.sh"
if [[ -f "$V240_TEST" ]]; then
  ok "G53: v2.4.0 test file exists"
else
  fail "G53: v2.4.0 test file missing"
fi

# G54. v2.4.0 scope test passes (fast, standalone)
if run_with_timeout 30 bash "$V240_TEST" >/dev/null 2>&1; then
  ok "G54: v2.4.0 scope test passes"
else
  note "G54: v2.4.0 scope test failed or timed out (non-blocking)"
fi

# ── Section H: Opt-in long regression ──────────────────────────────────────

echo ""
echo "=== Section H: Opt-in long regression ==="

if [[ "${NANOBK_RUN_LONG_REGRESSION:-0}" == "1" ]]; then
  echo "NANOBK_RUN_LONG_REGRESSION=1 — running legacy regression tests with timeout."

  # H55. v2.3.4 DNS apply engine test (timeout 60s)
  V234_TEST="$REPO_DIR/tests/v2.3.4-dns-apply-engine.sh"
  if [[ -f "$V234_TEST" ]]; then
    if run_with_timeout 60 bash "$V234_TEST" >/dev/null 2>&1; then
      ok "H55: v2.3.4 DNS apply engine test passes"
    else
      note "H55: v2.3.4 failed or timed out (environment-dependent, non-blocking)"
    fi
  else
    note "H55: v2.3.4 test not found"
  fi

  # H56. v2.3.6 cert issue gate test (timeout 60s)
  V236_TEST="$REPO_DIR/tests/v2.3.6-cert-issue-gate.sh"
  if [[ -f "$V236_TEST" ]]; then
    if run_with_timeout 60 bash "$V236_TEST" >/dev/null 2>&1; then
      ok "H56: v2.3.6 cert issue gate test passes"
    else
      note "H56: v2.3.6 failed or timed out (environment-dependent, non-blocking)"
    fi
  else
    note "H56: v2.3.6 test not found"
  fi

  # H57. v2.3.7 token rotation gate test (timeout 60s)
  V237_TEST="$REPO_DIR/tests/v2.3.7-token-rotation-gate.sh"
  if [[ -f "$V237_TEST" ]]; then
    if run_with_timeout 60 bash "$V237_TEST" >/dev/null 2>&1; then
      ok "H57: v2.3.7 token rotation gate test passes"
    else
      note "H57: v2.3.7 failed or timed out (environment-dependent, non-blocking)"
    fi
  else
    note "H57: v2.3.7 test not found"
  fi
else
