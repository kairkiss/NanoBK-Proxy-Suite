...
echo ""
echo "── ss unavailable branch safety ──"

SS_BLOCK="$(grep -n -A8 -B2 'ss 不可用' "$INSTALLER" || true)"
check "ss unavailable branch exists" "$(echo "$SS_BLOCK" | grep -q 'ss 不可用' && echo 1 || echo 0)"
...
sub_token_pattern="SUB""_TOKEN=[A-Za-z0-9]{20}"
admin_token_pattern="ADMIN""_TOKEN=[A-Za-z0-9]{20}"
blocked_ip="62.60"".""250.69"
check "no real SUB_TOKEN" "$(no_pattern "$INSTALLER" "$sub_token_pattern")"
check "no real ADMIN_TOKEN" "$(no_pattern "$INSTALLER" "$admin_token_pattern")"
check "no real VPS IP" "$(no_pattern "$INSTALLER" "$blocked_ip")"
