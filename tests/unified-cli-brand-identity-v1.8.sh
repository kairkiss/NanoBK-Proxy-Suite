#!/usr/bin/env bash
# NanoBK Proxy Suite — v1.8.10 CLI Brand Identity Test
#
...
echo ""
echo "=== Test Suite: v1.8.10 CLI Brand Identity ==="
...
output=$(source_ui_and_run "" "
  ui_banner 'v1.8.10' 'Full Recommended — VPS + Cloudflare + Bot + Web Panel'
")

assert_contains "$output" "NanoBK Proxy Suite" "Default: product name"
assert_contains "$output" "v1.8.10" "Default: version"
...
assert_contains "$plain_output" "v1.8.10" "PLAIN: version"
...
assert_contains "$noemoji_output" "v1.8.10" "NO_EMOJI: version"
...
assert_contains "$ui0_output" "v1.8.10" "UI=0: version"
...
assert_contains "$ci_output" "v1.8.10" "CI: version"
...
# Check longest line in default banner does not exceed 90 columns
max_width=$(echo "$output" | awk '{ print length }' | sort -rn | head -1)
if [[ "$max_width" -le 90 ]]; then
  pass "Width: longest line $max_width <= 90"
else
  fail "Width: longest line $max_width > 90"
fi
