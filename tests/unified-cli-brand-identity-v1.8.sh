#!/usr/bin/env bash
# NanoBK Proxy Suite — v1.8.25 CLI Brand Identity Test
#
...
echo ""
echo "=== Test Suite: v1.8.25 CLI Brand Identity ==="
...
output=$(source_ui_and_run "" "
  ui_banner 'v1.8.25' 'Full Recommended — VPS + Cloudflare + Bot + Web Panel'
")

assert_contains "$output" "NanoBK Proxy Suite" "Default: product name"
assert_contains "$output" "v1.8.25" "Default: version"
...
box_output=$(source_ui_and_run "" "
  _ui_banner_box 'v1.8.25' 'Full Recommended — VPS + Cloudflare + Bot + Web Panel'
")
...
assert_contains "$box_output" "v1.8.25" "Box: version"
...
long_output=$(source_ui_and_run "" "
  _ui_banner_box 'v1.8.25' 'Full Recommended — VPS + Cloudflare + Telegram Bot + Web Panel + Extra Long Subtitle For Width Guard'
")

assert_contains "$long_output" "NanoBK Proxy Suite" "Long: product name"
assert_contains "$long_output" "v1.8.25" "Long: version"
...
assert_contains "$plain_output" "v1.8.25" "PLAIN: version"
...
assert_contains "$noemoji_output" "v1.8.25" "NO_EMOJI: version"
...
assert_contains "$ui0_output" "v1.8.25" "UI=0: version"
...
assert_contains "$ci_output" "v1.8.25" "CI: version"
