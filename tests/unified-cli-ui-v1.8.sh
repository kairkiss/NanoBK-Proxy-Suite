#!/usr/bin/env bash
# NanoBK Proxy Suite — v1.8.36 CLI UI Test
#
...
echo ""
echo "=== Test Suite: v1.8.36 CLI UI ==="
echo ""
...
assert_contains "$nanobk_version" "1.8.36" "nanobk --version shows 1.8.36"

install_version=$(grep '^VERSION=' "${REPO_DIR}/installer/install.sh" | head -1)
assert_contains "$install_version" "1.8.36" "install.sh VERSION is 1.8.36"

bootstrap_version=$(grep '^BOOTSTRAP_VERSION=' "${REPO_DIR}/installer/bootstrap.sh" | head -1)
assert_contains "$bootstrap_version" "1.8.36" "bootstrap.sh BOOTSTRAP_VERSION is 1.8.36"
