#!/usr/bin/env bash
# NanoBK Proxy Suite — v1.8.36 Operation Log Real Version Fast Test
#
...
#
# Usage:
#   bash tests/unified-cli-operation-log-real-version-fast-v1.8.sh
...
echo ""
echo "=== Test Suite: v1.8.36 Real Version Fast Test ==="

# ── Helper: create override script ─────────────────────────────────────
make_override_script
register_cleanup "$OVERRIDE_SCRIPT"

# ── 1: default test mode does NOT trigger real pilot ────────────────────
...
echo ""
echo "--- 2: real pilot fast trigger ---"

PILOT_DIR=$(mktemp -d)
register_cleanup "$PILOT_DIR"

fast_out=$(NANOBK_OPLOG_REAL_PILOT=1 NANOBK_OPLOG_DIR="$PILOT_DIR" \
  NANOBK_TEST_OVERRIDE_SCRIPT="$OVERRIDE_SCRIPT" \
  bash "${REPO_DIR}/installer/install.sh" --mode test --defaults 2>&1 < /dev/null || true)

assert_contains "$fast_out" "operation-log real command pilot enabled" "2a Fast: pilot enabled"
assert_contains "$fast_out" "bin/nanobk --version" "2a Fast: contains command"
assert_contains "$fast_out" "operation-log real command pilot completed" "2a Fast: pilot completed"
...
if [[ -n "$real_log" ]]; then
  ...
  if [[ "$real_log_perms" == "600" ]]; then
    ...
    fail "2b Fast: log chmod 600 (got $real_log_perms)"
  fi
  log_content=$(cat "$real_log")
  assert_contains "$log_content" "NanoBK" "2b Fast: log contains version output"
else
  fail "2b Fast: log file not found"
fi

# ── 3: verbose shows version output ────────────────────────────────────
...
verbose_out=$(NANOBK_VERBOSE=1 NANOBK_OPLOG_REAL_PILOT=1 NANOBK_OPLOG_DIR="$PILOT_DIR" \
  NANOBK_TEST_OVERRIDE_SCRIPT="$OVERRIDE_SCRIPT" \
  bash "${REPO_DIR}/installer/install.sh" --mode test --defaults 2>&1 < /dev/null || true)

assert_contains "$verbose_out" "operation-log real command pilot enabled" "3a Verbose: pilot enabled"
assert_contains "$verbose_out" "operation-log real command pilot completed" "3a Verbose: pilot completed"
assert_contains "$verbose_out" "NanoBK" "3a Verbose: shows version output"
...
plain_out=$(NANOBK_PLAIN=1 NANOBK_OPLOG_REAL_PILOT=1 NANOBK_OPLOG_DIR="$PILOT_DIR" \
  NANOBK_TEST_OVERRIDE_SCRIPT="$OVERRIDE_SCRIPT" \
  bash "${REPO_DIR}/installer/install.sh" --mode test --defaults 2>&1 < /dev/null || true)

assert_contains "$plain_out" "operation-log real command pilot enabled" "4a PLAIN: pilot enabled"
assert_contains "$plain_out" "operation-log real command pilot completed" "4a PLAIN: pilot completed"
assert_contains "$plain_out" "Log:" "4a PLAIN: shows log path"
plain_pilot_lines=$(grep -E "real command pilot|Log:" <<< "$plain_out" || true)
...
ui0_out=$(NANOBK_UI=0 NANOBK_OPLOG_REAL_PILOT=1 NANOBK_OPLOG_DIR="$PILOT_DIR" \
  NANOBK_TEST_OVERRIDE_SCRIPT="$OVERRIDE_SCRIPT" \
  bash "${REPO_DIR}/installer/install.sh" --mode test --defaults 2>&1 < /dev/null || true)

assert_contains "$ui0_out" "operation-log real command pilot enabled" "5a UI=0: pilot enabled"
assert_contains "$ui0_out" "operation-log real command pilot completed" "5a UI=0: pilot completed"
assert_contains "$ui0_out" "Log:" "5a UI=0: shows log path"
ui0_pilot_lines=$(grep -E "real command pilot|Log:" <<< "$ui0_out" || true)
...
ci_out=$(CI=1 NANOBK_OPLOG_REAL_PILOT=1 NANOBK_OPLOG_DIR="$PILOT_DIR" \
  NANOBK_TEST_OVERRIDE_SCRIPT="$OVERRIDE_SCRIPT" \
  bash "${REPO_DIR}/installer/install.sh" --mode test --defaults 2>&1 < /dev/null || true)

assert_contains "$ci_out" "operation-log real command pilot enabled" "6a CI: pilot enabled"
assert_contains "$ci_out" "operation-log real command pilot completed" "6a CI: pilot completed"
assert_contains "$ci_out" "Log:" "6a CI: shows log path"
ci_pilot_lines=$(grep -E "real command pilot|Log:" <<< "$ci_out" || true)
...
full_dry_out=$(NANOBK_OPLOG_REAL_PILOT=1 NANOBK_TEST_MOCK=1 NANOBK_ASSUME_PORTS_FREE=1 \
  bash "${REPO_DIR}/installer/install.sh" --mode full --dry-run --defaults --lang zh 2>&1 < /dev/null || true)
assert_not_contains "$full_dry_out" "operation-log real command pilot enabled" "7a Full dry-run: pilot not triggered"
