// Sanitized NanoK-style primary subscription Worker.
//
// This example demonstrates:
// - public token-protected Clash/Mihomo subscription output
// - KV-backed profile storage
// - private admin read/update endpoints for a VPS rotation script
// - strict YAML string quoting and control-character validation
//
// No production tokens, UUIDs, passwords, keys, IPs, account IDs, or KV IDs are included.
...
// Optional upstream subscription passthrough. Replace in your deployment or leave unused.
...
// YAML 1.2 disallows these control characters in a stream.
// TAB, LF, and CR are excluded from this regular expression intentionally.
const YAML_BAD_CHARS = /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F]/;
...
function hasYamlControlChars(value) {
  return YAML_BAD_CHARS.test(String(value));
}

function cleanString(value, fieldName) {
  const text = String(value ?? '').normalize('NFC').trim();
  if (!text) throw new Error(`missing required string: ${fieldName}`);
  if (hasYamlControlChars(text)) {
    throw new Error(`invalid control character in ${fieldName}`);
  }
  return text;
}

function optionalCleanString(value, fallback, fieldName) {
  const text = value === undefined || value === null || value === '' ? fallback : value;
  return cleanString(text, fieldName);
}
...
function asPort(value, fieldName) {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error(`invalid port: ${fieldName}`);
  }
  return port;
}
