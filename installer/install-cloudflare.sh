...
write_local_env() {
  ...
  local admin_current_url="${ROUTE_URL}${ADMIN_CURRENT_PATH}"
  local admin_update_url="${ROUTE_URL}${ADMIN_PATH}"
...
ADMIN_CURRENT_URL=\"${admin_current_url}\"
ADMIN_UPDATE_URL=\"${admin_update_url}\"
...
print_next_steps() {
  ...
  if [[ "$DEPLOY_NANOB" == "1" ]] && [[ -n "$NANOB_ROUTE_URL" ]]; then
    ...
    echo "    ${NANOB_ROUTE_URL}${NANOB_PATH}?token=<hidden>"
    echo ""
    echo "  Direct subscription (nanok only):"
    [[ -n "$ROUTE_URL" ]] && echo "    ${ROUTE_URL}${SUB_PATH}?token=<hidden>"
  elif [[ -n "$ROUTE_URL" ]]; then
    echo "  Subscription URL:"
    echo "    ${ROUTE_URL}${SUB_PATH}?token=<hidden>"
  fi

  echo ""
  echo "  Full subscription URL saved to local secret file."
  echo "  To view it, inspect .cloudflare.local.env locally."
  echo "  Do not paste it into chat/logs/issues."
