...
v1.7.21 fixes Cloudflare deploy status callback mismatch and admin env auto-install. v1.7.22 disables legacy admin env in Full Wizard, writes mock verified env, and tightens dynamic Summary checks. Neither claims real VPS validation, Cloudflare validation, or production pass.
