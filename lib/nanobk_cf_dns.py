...
def _format_text(plan: DnsPlan) -> str:
    ...
    lines.append("    1. Run: nanobk cf dns apply --profile <profile> --api-env <api-env> --dry-run")
    lines.append("    2. Then: nanobk cf dns apply --profile <profile> --api-env <api-env> --check")
    lines.append("    3. Only after reviewing the disposable hostname, run with --yes.")
    lines.append("")
    lines.append("    apply is available but should be used only with a disposable test prefix first.")
    lines.append("    plan itself still performs no mutation.")
    lines.append("")
    return "\n".join(lines)
